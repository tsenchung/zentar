<script lang="ts">
	...
	import { client, TextAidExercise, type Exercise } from './exercises';

	let exercises = client.collection();
	...
	function refresh() {
		exercises = client.collection();
	}

	function createExercise() {
		client.save(newExercise);
	...
	function deleteExercise(exerciseToDelete: Exercise | undefined) {
		return () => {
			if(exerciseToDelete) {
				client.delete(exerciseToDelete.id);
				refresh();
			}
		}
	}
...
<div class="overflow-x-auto">
	<div class="flex">
		...
		<div>
			<button class="btn" on:click={showCreateExerciseModal}>New</button>
		</div>
	</div>
	<table class="table">
		<thead>
